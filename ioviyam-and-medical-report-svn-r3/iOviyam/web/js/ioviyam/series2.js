...
$(document).ready(function() {
    ...
    var queryStringType = getParameter(queryString, "querytype");
    var modalityparam = getParameter(queryString, "Modality");


    if ((typeof tmppatientid != 'undefined') && (typeof tmpstudyid != 'undefined')) {
        params = {
            "PatientID": tmppatientid,
            "StudyID": tmpstudyid,
            "Modality": modalityparam
        };
        localStorage.setItem("dpatientid", tmppatientid);
        localStorage.setItem("dstudyid", tmpstudyid);
        localStorage.setItem("dmodality", modalityparam);
    } else {
        params = {
            "PatientID": localStorage.getItem("dpatientid"),
            "StudyID": localStorage.getItem("dstudyid"),
            "Modality": localStorage.getItem("dmodality")
        };
    }



//    if ((typeof tmpstudyid != 'undefined') || (typeof tmppatientid != 'undefined')) {
//        patientid = null;
...
//                window.location.href = result + "/index.html#home?querytype=" + queryStringType;
//                $.mobile.loading("hide");
//            }
//        });
//    }
    function URLForwardBack() {
        var pathname = $(location).attr('href');
        var arrayurl = new Array();
        pathname = pathname.replace('//', '_');
        arrayurl = pathname.split('/');
//    var urlPath = "/" + arrayurl[1] + "/#Seriespage";
//    urlPath = urlPath.replace('_', '//');
        return arrayurl[0].toString().replace('_', '//');
    }
    $("#btnback").click(function() {
        $.ajax({
            ...
            data: "",
            success: function(result) {
                window.location.href = URLForwardBack()+"/"+result + "/index.html#home?querytype=" + queryStringType;
                $.mobile.loading("hide");
            }
        });
    ...
    function loadSeries(data) {
        ...
        $.each(obj, function(i, row) {
            ...
            $('#serieslist').append('<li id=serieslistvalue' + i + ' data-theme="a"  ibodypart=' + row['bodyPart'] + ',' + row['seriesDesc'] + ' >'
                    ...
                    + '<img  id="imgstudy" style="float:left;"  src=' + outofnull[0] + '&frameNumber=1&rowsn=82&coloumns=82&type=jpg'+ '>'
                    +
        ...
        $('#serieslist').children('li').on('click', function() {
            ...
            //console.log(seriesList[patient]['studyUID'] + " lsdfjklsdfjlds");
