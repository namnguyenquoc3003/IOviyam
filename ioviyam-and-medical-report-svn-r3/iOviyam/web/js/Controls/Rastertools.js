...
function getXY(radious, ox, oy, angle) {
    var point = {
        x: 0, y: 0
    };
    point.x = ox + (radious) * Math.cos(angle*Math.PI / 180);
    point.y = oy + (radious) * Math.sin(angle*Math.PI / 180);
    return point;
}
...
function drawMeasure(context) {
    ...
    var x1, y1, x2, y2, lineDist = 50;


    x1 = pointData.startA.x;
    y1 = pointData.startA.y;
    x2 = pointData.startB.x;
    y2 = pointData.startB.y;
    var hy1 = $(document).height() - 1;
    var wx1 = $(document).width() - 1;
    var angle = Math.atan2(y2 - y1, x2 - x1)* 180 / Math.PI;
    var angle2 = Math.atan2(y1 - y2, x1 - x2)* 180 / Math.PI;

    var pointsDataFromAnlgle = getXY((circlesize*2), x1, y1, angle);
    var pointsDataFromAnlglex = getXY((circlesize*2), x2, y2, angle2);

    pointData.startA.x = pointsDataFromAnlgle.x;
    pointData.startA.y = pointsDataFromAnlgle.y;
    pointData.startB.x = pointsDataFromAnlglex.x;
    pointData.startB.y = pointsDataFromAnlglex.y;

//    pointData.startA.x = x1;
//    pointData.startA.y = y1;
//    pointData.startB.x = x2;
//    pointData.startB.y = y2;
...
function drawMeasureSinglemove(context) {
    ...
    if (pointA) {
      ...
      var x2=pointData.startB.x;
var y2=pointData.startB.y;
var x1=currentPointx;
var y1=currentPointy;
          var angle2 = Math.atan2(y2 - y1, x2 - x1)* 180 / Math.PI;

    var pointsDataFromAnlgle = getXY((circlesize*1.4), x1, y1, angle2);


currentPointx=pointsDataFromAnlgle.x;
currentPointy=pointsDataFromAnlgle.y;
    ...
    {   
      ...
      var x2=pointData.startA.x;
var y2=pointData.startA.y;
var x1=currentPointx;
var y1=currentPointy;
...
function  calline(x1, y1, x2, y2) {
    console.log("__X_" + nativerow1);
    var mult = 1;
    var height = stageheight * currentzoom;
    var ratio = height / nativerow1;
